ninja.wallets.detailwallet = {
		...
		if (ninja.privateKey.isBIP38Format(key)) {
			...
			ninja.privateKey.BIP38EncryptedKeyToByteArrayAsync(key, passphrase, function (rvnKeyOrError) {
				...
				if (rvnKeyOrError.message) {
					...
					ninja.wallets.detailwallet.populateKeyDetails(new Criptoreal.ECKey(rvnKeyOrError));
				}
			});
		}
		else {
			if (Criptoreal.ECKey.isMiniFormat(key)) {
			...
			else if (Criptoreal.ECKey.isBase6Format(key)) {
	...
	populateKeyDetails: function (rvnKey) {
		if (rvnKey.priv != null) {
			...
			var criptorealAddress = rvnKey.getCriptorealAddress();
			var wif = rvnKey.getCriptorealWalletImportFormat();
			document.getElementById("detailpubkey").innerHTML = rvnKey.getPubKeyHex();
			document.getElementById("detailaddress").innerHTML = criptorealAddress;
			document.getElementById("detailprivwif").innerHTML = wif;
			rvnKey.setCompressed(true);
			var criptorealAddressComp = rvnKey.getCriptorealAddress();
			var wifComp = rvnKey.getCriptorealWalletImportFormat();
			document.getElementById("detailpubkeycomp").innerHTML = rvnKey.getPubKeyHex();
			document.getElementById("detailaddresscomp").innerHTML = criptorealAddressComp;
			document.getElementById("detailprivwifcomp").innerHTML = wifComp;

			ninja.qrCode.showQrCode({
				"detailqrcodepublic": criptorealAddress,
				"detailqrcodepubliccomp": criptorealAddressComp,
