(function (ninja) {
	var ut = ninja.unitTests = {
		...
		synchronousTests: {
			...
			testGetCriptorealAddressFromByteArray: function () {
				var bytes = [4, 120, 152, 47, 64, 250, 12, 11, 122, 85, 113, 117, 131, 175, 201, 154, 78, 223, 211, 1, 162, 114, 157, 197, 155, 11, 142, 185, 225, 134, 146, 188, 181, 33, 240, 84, 250, 217, 130, 175, 76, 193, 147, 58, 253, 31, 27, 86, 62, 167, 121, 166, 170, 108, 206, 54, 163, 11, 148, 125, 214, 83, 230, 62, 68];
				var address = ninja.publicKey.getCriptorealAddressFromByteArray(bytes);
			...
			testGetByteArrayFromMultiplying: function () {
				...
				var bytes = ninja.publicKey.getByteArrayFromMultiplying(key1, new Criptoreal.ECKey(key2));
			...
			testGetByteArrayFromMultiplyingCompressedOutputsCompressed: function () {
				...
				var ecKey = new Criptoreal.ECKey(key2);
			...
			testGetPubHexFromMultiplyingPrivAPubB: function () {
				...
				var bytes = ninja.publicKey.getByteArrayFromMultiplying(keyPub, new Criptoreal.ECKey(keyPriv));
			...
			testBadKeyIsNotWif: function () {
				return !(Criptoreal.ECKey.isWalletImportFormat("bad key"));
			},
			testBadKeyIsNotWifCompressed: function () {
				return !(Criptoreal.ECKey.isCompressedWalletImportFormat("bad key"));
			},
			testBadKeyIsNotHex: function () {
				return !(Criptoreal.ECKey.isHexFormat("bad key"));
			},
			testBadKeyIsNotBase64: function () {
				return !(Criptoreal.ECKey.isBase64Format("bad key"));
			},
			testBadKeyIsNotMini: function () {
				return !(Criptoreal.ECKey.isMiniFormat("bad key"));
			},
			testBadKeyReturnsNullPrivFromECKey: function () {
				var key = "bad key";
				var ecKey = new Criptoreal.ECKey(key);
				if (ecKey.priv != null) {
					return false;
				}
				return true;
			},
			testGetCriptorealPrivateKeyByteArray: function () {
				...
				var rvnKey = new Criptoreal.ECKey(key);
				if (rvnKey.getCriptorealPrivateKeyByteArray().toString() != bytes.toString()) {
					return false;
				}
				return true;
			},
			testECKeyDecodeWalletImportFormat: function () {
				...
				var bytes2 = Criptoreal.ECKey.decodeWalletImportFormat(key);
			...
			testECKeyDecodeCompressedWalletImportFormat: function () {
				...
				var bytes1 = [41, 38, 101, 195, 135, 36, 24, 173, 241, 218, 127, 250, 58, 100, 111, 47, 6, 2, 36, 109, 166, 9, 138, 145, 210, 41, 195, 33, 80, 242, 113, 139];
				var bytes2 = Criptoreal.ECKey.decodeCompressedWalletImportFormat(key);
				if (bytes1.toString() != bytes2.toString()) {
					return false;
				}
				return true;
			...
			testBase64ToECKey: function () {
				...
				var rvnKey = new Criptoreal.ECKey(key);
				if (rvnKey.getCriptorealBase64Format() != "KSZlw4ckGK3x2n/6OmRvLwYCJG2mCYqR0inDIVDycYs=") {
					return false;
				}
				return true;
			},
			testHexToECKey: function () {
				...
				var rvnKey = new Criptoreal.ECKey(key);
				if (rvnKey.getCriptorealHexFormat() != "292665C3872418ADF1DA7FFA3A646F2F0602246DA6098A91D229C32150F2718B") {
					return false;
				}
				return true;
			},
			testCompressedWifToECKey: function () {
				var key = "KxbhchnQquYQ2dfSxz7rrEaQTCukF4uCV57TkamyTbLzjFWcdi3S";
				var rvnKey = new Criptoreal.ECKey(key);
				if (rvnKey.getCriptorealWalletImportFormat() != "KxbhchnQquYQ2dfSxz7rrEaQTCukF4uCV57TkamyTbLzjFWcdi3S"
						|| rvnKey.getPubPoint().compressed != true) {
					return false;
				}
				return true;
			...
			testWifToECKey: function () {
				var key = "5J8QhiQtAiozKwyk3GCycAscg1tNaYhNdiiLey8vaDK8Bzm4znb";
				var rvnKey = new Criptoreal.ECKey(key);
				if (rvnKey.getCriptorealWalletImportFormat() != "5J8QhiQtAiozKwyk3GCycAscg1tNaYhNdiiLey8vaDK8Bzm4znb") {
					return false;
				}
				return true;
			},
			testBrainToECKey: function () {
				...
				var rvnKey = new Criptoreal.ECKey(bytes);
				if (rvnKey.getCriptorealWalletImportFormat() != "5J8QhiQtAiozKwyk3GCycAscg1tNaYhNdiiLey8vaDK8Bzm4znb") {
					return false;
				}
				return true;
			...
			testMini30CharsToECKey: function () {
				...
				var rvnKey = new Criptoreal.ECKey(key);
				if (rvnKey.getCriptorealWalletImportFormat() != "5JrBLQseeZdYw4jWEAHmNxGMr5fxh9NJU3fUwnv4khfKcg2rJVh") {
					return false;
				}
				return true;
			},
			testGetECKeyFromAdding: function () {
				...
				if (ecKey.getCriptorealWalletImportFormat() != "5KAJTSqSjpsZ11KyEE3qu5PrJVjR4ZCbNxK3Nb1F637oe41m1c2") {
					return false;
				}
				return true;
			},
			testGetECKeyFromAddingCompressed: function () {
				...
				var key2 = "L1n4cgNZAo2KwdUc15zzstvo1dcxpBw26NkrLqfDZtU9AEbPkLWu";
				var ecKey = ninja.privateKey.getECKeyFromAdding(key1, key2);
				if (ecKey.getCriptorealWalletImportFormat() != "L3A43j2pc2J8F2SjBNbYprPrcDpDCh8Aju8dUH65BEM2r7RFSLv4") {
					return false;
				}
				return true;
			},
			...
			testGetECKeyFromMultiplying: function () {
				...
				if (ecKey.getCriptorealWalletImportFormat() != "5KetpZ5mCGagCeJnMmvo18n4iVrtPSqrpnW5RP92Gv2BQy7GPCk") {
					return false;
				}
				return true;
			},
			testGetECKeyFromMultiplyingCompressed: function () {
				...
				var key2 = "L1n4cgNZAo2KwdUc15zzstvo1dcxpBw26NkrLqfDZtU9AEbPkLWu";
				var ecKey = ninja.privateKey.getECKeyFromMultiplying(key1, key2);
				if (ecKey.getCriptorealWalletImportFormat() != "L5LFitc24jme2PfVChJS3bKuQAPBp54euuqLWciQdF2CxnaU3M8t") {
					return false;
				}
				return true;
			},
			...
			testGetECKeyFromBase6Key: function () {
				...
				var ecKey = new Criptoreal.ECKey(baseKey);
				if (ecKey.getCriptorealHexFormat() != hexKey) {
					return false;
				}
				return true;
			},
			...
			testBugWithLeadingZeroBytePublicKey: function () {
				...
				var rvnKey = new Criptoreal.ECKey(key);
				if (rvnKey.getCriptorealAddress() != "1M6dsMZUjFxjdwsyVk8nJytWcfr9tfUa9E") {
					return false;
				}
				return true;
			},
			testBugWithLeadingZeroBytePrivateKey: function () {
				...
				var rvnKey = new Criptoreal.ECKey(key);
				if (rvnKey.getCriptorealAddress() != "1NAjZjF81YGfiJ3rTKc7jf1nmZ26KN7Gkn") {
					return false;
				}
				return true;
			}
		...
		asynchronousTests: {
			...
			testBip38: function (done) {
				...
				var decryptTest = function (test, i, onComplete) {
					ninja.privateKey.BIP38EncryptedKeyToByteArrayAsync(test[0], test[1], function (privBytes) {
						if (privBytes.constructor == Error) {
							...
							var rvnKey = new Criptoreal.ECKey(privBytes);
							var wif = !test[2].substr(0, 1).match(/[LK]/) ? rvnKey.setCompressed(false).getCriptorealWalletImportFormat() : rvnKey.setCompressed(true).getCriptorealWalletImportFormat();
							if (wif != test[2]) {
				...
				var cycleTest = function (i, compress, onComplete) {
					...
					var privKey = (new Criptoreal.ECKey(false)).getCriptorealWalletImportFormat();
					...
					ninja.privateKey.BIP38PrivateKeyToEncryptedKeyAsync(privKey, 'testing', compress, function (encryptedKey) {
						...
						ninja.privateKey.BIP38EncryptedKeyToByteArrayAsync(encryptedKey, 'testing', function (decryptedBytes) {
							var decryptedKey = (new Criptoreal.ECKey(decryptedBytes)).getCriptorealWalletImportFormat();
						...
						ninja.privateKey.BIP38GenerateECAddressAsync(intermediatePoint, false, function (address, encryptedKey) {
							ninja.privateKey.BIP38EncryptedKeyToByteArrayAsync(encryptedKey, pass, function (privBytes) {
								if (privBytes.constructor == Error) {
									...
									var rvnKey = new Criptoreal.ECKey(privBytes);
									var rvnAddress = rvnKey.getCriptorealAddress();
									if (address !== rvnKey.getCriptorealAddress()) {
