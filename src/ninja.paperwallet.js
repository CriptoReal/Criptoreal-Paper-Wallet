ninja.wallets.paperwallet = {
	...
	// generate criptoreal address, private key, QR Code and update information in the HTML
	// idPostFix: 1, 2, 3, etc.
	generateNewWallet: function (idPostFix) {
		if (ninja.wallets.paperwallet.encrypt) {
			...
			ninja.wallets.paperwallet.showArtisticWallet(idPostFix, criptorealAddress, privateKeyWif);
		}
	},
	...
	testAndApplyVanityKey: function () {
		...
		if (!ninja.privateKey.isPrivateKey(suppliedKey)) {
			...
			var computedPublicAddress = new Criptoreal.ECKey(suppliedKey).getCriptorealAddress();
			if (ninja.wallets.paperwallet.encrypt) {
				document.getElementById("busyblock").className = "busy";
	...
	showArtisticWallet: function (idPostFix, criptorealAddress, privateKey) {
		var keyValuePair = {};
		keyValuePair["qrcode_public" + idPostFix] = criptorealAddress;
        ...
        document.getElementById("rvnaddress" + idPostFix).innerHTML = criptorealAddress;
